...
sys.path.append(str(Path.cwd() / "waveglow"))
...
class Vocoder():
    def __init__(self, path):
        ...
        model = torch.load(path, map_location='cpu')['model'].eval()
        self.net = model.remove_weightnorm(model)

    @torch.no_grad()
    def inference(self, spect: torch.Tensor):
        self.net = self.net.to(spect.device)
        ...
        for k in reversed(range(self.net.n_flows)):
            ...
            if k % self.net.n_early_every == 0 and k > 0:
                z = torch.randn(spect.size(0), self.net.n_early_size, spect.size(2), device=spect.device)
